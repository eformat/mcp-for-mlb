...
def run_tuning_op(
    ...
    def fetch_predictions():
        ...
        cur.execute("""
            ...
            WHERE was_correct IS NOT NULL ORDER BY game_date
        """)
        columns = [d[0] for d in cur.description]
        rows = cur.fetchall()
        conn.close()
        result = [dict(zip(columns, row)) for row in rows]
        print(f"  Found {len(result)} resolved predictions", flush=True)
        return result
...
"""
    ...
    # Start MLflow run
    mlflow.start_run(run_name=f"tune-{agent_model}")
...
def mlb_prompt_tuning_pipeline(
    ...
    mlflow_experiment_name: str = "mlb-prompt-tuning",
    agent_model: str = "qwen38-27b",
    llm_base_url: str = "https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/qwen38-27b/v1",
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        output_file = output_dir / "mlb-tune-pipeline.yaml"
