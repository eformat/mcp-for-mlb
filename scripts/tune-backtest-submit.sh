#!/usr/bin/env bash
# Compile and submit a backtest-based prompt tuning pipeline run.
set -euo pipefail
...
PIPELINE_FILE="${REPO_DIR}/prompt_tuning/pipelines_gen/mlb-tune-backtest-pipeline.yaml"
...
run = client.create_run_from_pipeline_package(
    ...
    run_name='tune-bt-$(date +%Y%m%d-%H%M%S)',
    experiment_name='mlb-prompt-tuning-backtest',
    arguments={
        'mlflow_tracking_uri': 'https://mlflow.redhat-ods-applications.svc:8443/mlflow',
        'mlflow_workspace': '${NAMESPACE}',
        'mlflow_experiment_name': 'mlb-prompt-tuning-backtest',
        'agent_model': '${AGENT_MODEL:-qwen36-27b}',
        'llm_base_url': 'https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/${AGENT_MODEL:-qwen36-27b}/v1',
